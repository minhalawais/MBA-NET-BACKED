...
def add_extra_income(data, user_role, current_user_id, ip_address, user_agent):
    try:
        ...
        try:
            income_datetime = datetime.strptime(f"{income_date_str} {income_time_str}", "%Y-%m-%d %H:%M")
        except ValueError:
...
def update_extra_income(id, data, company_id, user_role, current_user_id, ip_address, user_agent):
    try:
        ...
        # Handle income_date separately to combine date and time
        if 'income_date' in data and 'income_time' in data:
            income_datetime = datetime.strptime(f"{data['income_date']} {data['income_time']}", "%Y-%m-%d %H:%M")
            income.income_date = income_datetime
